...
<body class="min-h-screen p-8">
  ...
  <div class="max-w-5xl mx-auto">
    ...
    <!-- Stats bar -->
    <div id="stats" class="hidden grid grid-cols-4 gap-4 mb-8">
    ...
    <div id="empty" class="hidden text-center text-gray-600 py-24">
      ...
      <p class="font-medium">No memories found for this user.</p>
    </div>
...
<script>
  ...
  async function load() {
    ...
    try {
      const r = await fetch(`/memories?userId=${encodeURIComponent(uid)}&limit=500`);
  ...
  function render() {
    ...
    let mems = allMemories.filter(m => !cat || m.category === cat);
    ...
    // Stats
    const strong   = allMemories.filter(m => m.strength >= 0.5).length;
    const fading   = allMemories.filter(m => m.strength >= 0.05 && m.strength < 0.5).length;
    const critical = allMemories.filter(m => m.strength < 0.1).length;

    document.getElementById('statTotal').textContent    = allMemories.length;
  ...
  // Auto-load from ?user= query param
